...
class compiler : public regex_constants {
 public:
  ...
  optional<matcher_type> compile();

 private:
  compiler_impl<Source> impl;
...
template <typename Source>
optional<typename compiler<Source>::matcher_type>
compiler<Source>::compile() {
  return impl.get_alternation();
}

/*
template <typename Source>
optional<typename compiler<Source>::matcher_type>
compiler<Source>::get_alternation() {
  using std::forward;
  using std::move;
...
template <typename Source>
optional<typename compiler<Source>::matcher_type>
compiler<Source>::get_expression() {
  using std::move;
...
compiler<Source>::get_element() {
  ...
  case token_type::STRING_LITERAL:
    return string_matcher(std::begin(token->str), std::end(token->str));
...
compiler<Source>::get_replication() {
  auto token = source.get();
  if (!token) return {};
...
template <typename Source>
optional<typename compiler<Source>::matcher_type> 
compiler<Source>::get_subexpression() {
  auto token = source.get();
  if (!token) { 
    return {};
  }
...
template <typename Source>
template <typename Compositor>
typename compiler<Source>::matcher_type
compiler<Source>::compose(std::vector<matcher_type>&& elements,
    Compositor compositor) {
  using std::move;
...
template <typename Source>
bool compiler<Source>::eat_token(token_type type) {
  auto token = source.get();
